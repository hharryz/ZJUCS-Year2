...
image_array = imread('absolute path of the image');
...
% 蓝色分量右移4位取出高4位,左移0位作为ROM中RGB数据的第3bit到第0bit
 for i = 1:height*width
 	rgb(i) = bitshift(bitshift(r(i),-4),0)+ bitshift(bitshift(g(i),-4),4)+ bitshift(bitshift(b(i),-4),8);
 end


fid = fopen( 'absolute path of /___.coe', 'w+' );		% // 这里的地址需要写成绝对路径 不然有报错
